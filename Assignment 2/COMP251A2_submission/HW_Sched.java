...
public class HW_Sched {
	...
	public int[] SelectAssignments() {
		...
		//YOUR CODE GOES HERE
		int[] filled_times = new int[lastDeadline+1];
		for(int x=0;x< Assignments.size();x++) {
			int  y=0;
			Assignment active_a = Assignments.get(x); 
			int latest = active_a.deadline; 
			while(y==0){
				if(latest ==0){
					break;
					
				}
				if(filled_times[latest]==0){
					filled_times[latest]=1;
					homeworkPlan[active_a.number] = latest;
					break;
				}
				latest--;
			}
	...
	public static boolean contains(int[] container, int search_val) {
